...
describe('CurrencyProfitWidget', () => {
  ...
  it('should receive correct prop values', () => {
    expect(wrapper.find('#date').text()).toEqual(formatDate(mockProps.date));
    expect(wrapper.find('#profit').text()).toEqual('Profit: ' + formatCurrencyProfit(mockProps.currencyProfit));

    expect(wrapper.find('#currency').text()).toEqual('BTC');
    expect(wrapper.find('#bestBuyingPrice').text()).toEqual(
      formatBuyingPrice(mockProps.currencyProfitDetails.bestBuyingPrice)
    );
    expect(wrapper.find('#bestSellingPrice').text()).toEqual(
      formatSellingPrice(mockProps.currencyProfitDetails.bestSellingPrice)
    );
